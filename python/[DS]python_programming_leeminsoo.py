...
def data_type(values):
    print(type(values), end='')
    print('|', end='')
    for value in values:
        print(type(value), end='')
        if isinstance(value, list):
            print('|', end='')
            for smallvalue in value:
                print(type(smallvalue), end='')
            print('|', end='')
    print('|')

print("2번 답안")
data_type([2, 3, [2, '3'], 3.5, "Hello"])
...
def calc_tips():
    ...
    if ratio_tip == '':
        ...
        ratio_tip = int(ratio_tip) / 100
    total_tip = math.ceil(total_fee * ratio_tip)
    print("팁: " + str(total_tip) + '달러')
...
def search_target(sentence, target):
    match_count = sentence.count(target)
    match_index = [i for i in range(len(sentence)) if sentence.startswith(target, i)]
    print(str(match_count) + ', ' + str(match_index))
print("4번 답안")
...
def div_ab(a, b):
    if a.isdecimal() and b.isdecimal():
        ...
        print(init_a + '나누기 ' + str(b) + '의 결과: 몫 ' + str(count) + ', 나머지 ' + str(a))
    else:
        print('정수만 입력해주세요.')
print("5번 답안")
a = '37'
...
# 문제 6 (25점)
def hanoi_sol(number_of_disk, start_point, end_point, reference_point):
    if number_of_disk == 1:
        print(MSG_FORMAT.format(start_point, end_point))
        return
    else:
        hanoi_sol(number_of_disk - 1, start_point, reference_point, end_point)
        print(MSG_FORMAT.format(start_point, end_point))
        hanoi_sol(number_of_disk - 1, reference_point, end_point, start_point)
...
reference_point = 'B'
end_point = 'C'
hanoi_sol(number_of_disk, start_point, end_point, reference_point)
